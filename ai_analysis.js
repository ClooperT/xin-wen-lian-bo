...
function buildMessage(title, content, icon) {
  ...
  for (const line of lines) {
    ...
    // Check if this line starts with a step marker
    let isStep = false;
    let stepIcon = '';
    for (const [key, val] of Object.entries(stepNames)) {
      if (cleaned.startsWith(key) || cleaned.includes(key)) {
        const afterStep = cleaned.replace(new RegExp(key + '[：:\\s]*'), '').split('|').map(s => s.trim()).filter(s => s);
        if (afterStep.length > 0) {
          msgContent.push([{ tag: 'text', text: '' }]);
          msgContent.push([{ tag: 'text', text: val }]);
          msgContent.push([{ tag: 'text', text: '  ' + afterStep.join(' ┊ ') }]);
        } else {
          msgContent.push([{ tag: 'text', text: '' }]);
          msgContent.push([{ tag: 'text', text: val }]);
        }
    ...
    // Regular content line - split by | and format
    if (cleaned.includes('|')) {
      const parts = cleaned.split('|').map(s => s.trim()).filter(s => s);
      // Remove any remaining step number references
      const cleanParts = parts.map(p => p.replace(/第[一二三四五六七八九十]+步/g, '').replace(/[：:]\s*/g, ' ').trim()).filter(s => s);
      msgContent.push([{ tag: 'text', text: cleanParts.join(' ┊ ') }]);
    } else {
      // Plain text, add with proper indent
      const t = cleaned.replace(/第[一二三四五六七八九十]+步[：:]\s*/g, '').trim();
      if (t) {
        msgContent.push([{ tag: 'text', text: '  ' + t }]);
      }
    }
  }
