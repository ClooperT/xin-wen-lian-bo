  ...
  const resp = await fetch('https://api.deepseek.com/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        ...
        { role: 'user', content: '以下是今日《新闻联播》完整文字稿，请按流程分析：\n\n' + text }
      ],
      temperature: 0.3,
      max_tokens: 2000
    })
...
function buildMessage(analysis) {
  ...
  const content = [];
  content.push([{ tag: 'text', text: '🔍 今日产业信号分析' }]);
  ...
  content.push([{ tag: 'text', text: '' }]);
  content.push([{ tag: 'text', text: '—————————————————' }]);
  content.push([{ tag: 'text', text: '由 DeepSeek AI 分析生成' }]);
  return {
    msg_type: 'post',
    content: {
      post: {
        zh_cn: {
          title: '🔍 《新闻联播》产业信号分析 (' + DATE + ')',
          content: content
        }
      }
    }
...
async function sendMessage(msg) {
  ...
  if (!resp.ok) {
    const err = await resp.text();
    throw new Error('Feishu error: ' + resp.status + ' ' + err);
  }
...
async function main() {
  if (!DEEPSEEK_KEY) {
    console.log('未设置 DEEPSEEK_API_KEY，跳过 AI 分析');
    process.exit(0);
  }
  if (!WEBHOOK_URL) {
    console.error('错误: 未设置 FEISHU_WEBHOOK');
    process.exit(1);
  }
  if (!fs.existsSync(NEWS_PATH)) {
    console.log('新闻文件不存在，跳过 AI 分析');
    process.exit(0);
  }

  const md = fs.readFileSync(NEWS_PATH, 'utf-8');
  const plainText = md.replace(/<[^>]*>/g, '').trim();
  const truncated = plainText.length > 8000 ? plainText.substring(0, 8000) + '\n\n[内容较长，已截取前8000字符]' : plainText;

  console.log('正在调用 DeepSeek 分析产业信号...');
  console.log('新闻文本长度: ' + truncated.length + ' 字符');
  const analysis = await analyzeNews(truncated);
  console.log('分析完成，共 ' + analysis.length + ' 字');

  const msg = buildMessage(analysis);
  await sendMessage(msg);
  console.log('✅ 产业分析已推送到飞书');
}
